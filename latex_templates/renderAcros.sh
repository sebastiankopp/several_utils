...
# Compute max width
widest_abbrev=$(sed -rs 's/#.+$//' "$infile" | grep '\S\+' | awk -F= '{print length($1)" "$1}' | sort -n | tail -n 1 | awk '{print $2}') || exit 44

echo "Widest abbrevation: $widest_abbrev"

echo "\\begin{acronym}[${widest_abbrev}]" > "$outfile" || exit 44
sed -rs 's/#.+$//' "$infile" | grep '\S\+' | sort | awk -F= '{print "\\acro{"$1"}["$1"]{"$2"}"}' >> "$outfile" || exit 44
